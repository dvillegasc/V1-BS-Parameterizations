...
server <- function(input, output, session) {
  ...
  resultados_fase1 <- eventReactive(c(input$analyze, valores$omitidos), {
    ...
    validate(need(datos_finales_1(), "⚠️ Error de lectura: No se encontraron datos numéricos. Verifique el separador."))
    ...
    # Ajuste GAMLSS
    datos_est <- as.vector(as.matrix(df[validos, ])); datos_est <- datos_est[!is.na(datos_est)]
    
    mod <- try(gamlss(datos_est ~ 1, family = BS6, control = gamlss.control(trace=FALSE)), silent=TRUE)
    
    if (class(mod)[1] == "try-error") { mu <- mean(datos_est); sigma <- sd(datos_est) }
    else { mu <- as.numeric(fitted(mod,"mu")[1]); sigma <- as.numeric(fitted(mod,"sigma")[1]) 
    if(is.na(mu)) {mu<-mean(datos_est); sigma<-sd(datos_est)} }
    
    # Simulación Bootstrap
    if(exists("rBS6")) boots <- matrix(rBS6(n*B, mu, sigma), B, n)
    else boots <- matrix(rnorm(n*B, mu, sigma), B, n)
  ...
  output$chart_fase1 <- renderPlot({
    ...
    ggplot(df, aes(ID, Media)) +
      ...
      labs(title="Carta de diseño y ajuste", subtitle="Haga clic para omitir puntos.", y="Media muestral", x="Subgrupos") +
  ...
  output$tabla_fase1 <- renderTable({ 
    validate(need(datos_finales_1(), "No hay datos válidos para mostrar.")); 
  ...
  output$chart_fase2 <- renderPlot({
    ...
    ggplot(dfg, aes(ID, Media)) +
      ...
      labs(title="Carta de monitoreo", subtitle=paste("Límites fijos (n =", valores$n_fase1, ")"), y="Media muestral", x="Subgrupos")
  })
