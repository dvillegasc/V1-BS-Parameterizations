...
gendat <- function(n) {
  x1 <- runif(n)
  x2 <- runif(n)
  mu    <- exp(0.2 + 0.4 * x1) # 5 approximately
  sigma <- exp(0.3 + 0.4 * x2) #  5  approximately
...
parSim(
  ...
  name = "C:/Users/davil/Desktop/BS-Parametrizations/Simulaciones/Simuls/BS9_Sim_Cov",# Name of the file
  nCores = 6,                       # Number of cores to use
  progressbar = TRUE,               # Progress bar
  export = c("gendat", "dBS", "pBS", "qBS", "rBS", "hBS", "dBS9", "pBS9", "qBS9", "rBS9", "hBS9", "BS9"),
  ...
  expression = {
    ...
    f   <- y ~ x1 | x2 
    mod <- try(suppressMessages(gamlss2(f, family=BS9, data=dat)), silent = TRUE)
...
archivos <- list.files(pattern = "^BS9_Sim_Cov.*\\.txt$", 
...
ggsave(filename = "C:/Users/davil/Desktop/BS-Parametrizations/Simulaciones/Figs/bias_mse_BS9_Sim_Cov.pdf", 
