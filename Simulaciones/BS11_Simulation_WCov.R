...
parSim(
  ...
  name = "C:/Users/davil/Desktop/BS-Parametrizations/Simulaciones/Simuls/BS11_Sim_WCov",  # Name of the file
  nCores = 6,                                 # Number of cores to use
  progressbar = TRUE,               # Progress bar
  export = c("dBS", "pBS", "qBS", "rBS", "hBS", "dBS11", "pBS11", "qBS11", "rBS11", "hBS11", "BS11"),
  ...
  expression = {
    ...
    y <- rBS11(n=n, mu, sigma)
    
    f   <- y ~ 1
    mod <- try(suppressMessages(gamlss2(f, family=BS11)), silent = TRUE)
...
archivos <- list.files(pattern = "^BS11_Sim_WCov.*\\.txt$", 
...
ggsave(filename="C:/Users/davil/Desktop/BS-Parametrizations/Simulaciones/Figs/bias_BS11_Sim_WCov.pdf", width=12, height=6,
