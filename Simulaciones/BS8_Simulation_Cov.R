...
gendat <- function(n) {
  x1 <- runif(n)
  x2 <- runif(n)
  mu    <- exp(2.5 + 1 * x1) # 5 approximately
  sigma <- exp(0.2 + 0.4 * x2) #  5  approximately
...
parSim(
  ...
  name = "C:/Users/davil/Desktop/BS-Parametrizations/Simulaciones/Simuls/BS8_Sim_Cov",# Name of the file
  nCores = 6,                       # Number of cores to use
  progressbar = TRUE,               # Progress bar
  export = c("gendat", "dBS", "pBS", "qBS", "rBS", "hBS", "dBS8", "pBS8", "qBS8", "rBS8", "hBS8", "BS8"),
  ...
  expression = {
    ...
    f   <- y ~ x1 | x2 
    mod <- try(suppressMessages(gamlss2(f, family=BS8, data=dat)), silent = TRUE)
...
archivos <- list.files(pattern = "^BS8_Sim_Cov.*\\.txt$", 
...
dat <- datos %>% group_by(n) %>% 
  summarise(nobs = n(),
            ...
            bias_b0 = mean(beta_0_hat - (2.5), trim=trim, na.rm=TRUE),
            bias_b1 = mean(beta_1_hat - (1.0), trim=trim, na.rm=TRUE),
            bias_g0 = mean(gamma_0_hat - (0.2), trim=trim, na.rm=TRUE),
            bias_g1 = mean(gamma_1_hat - (0.4), trim=trim, na.rm=TRUE),
            
            mse_b0 = mean((beta_0_hat - (2.5))^2, trim=trim, na.rm=TRUE),
            mse_b1 = mean((beta_1_hat - (1.0))^2, trim=trim, na.rm=TRUE),
...
ggsave(filename = "C:/Users/davil/Desktop/BS-Parametrizations/Simulaciones/Figs/bias_mse_BS8_Sim_Cov.pdf", 
