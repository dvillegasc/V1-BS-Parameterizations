...
parSim(
  ...
  name = "C:/Users/davil/Desktop/BS-Parametrizations/Simulaciones/Simuls/BS9_Sim_WCov",  # Name of the file
  nCores = 6,                                 # Number of cores to use
  progressbar = TRUE,               # Progress bar
  export = c("dBS", "pBS", "qBS", "rBS", "hBS", "dBS9", "pBS9", "qBS9", "rBS9", "hBS9", "BS9"),
  ...
  expression = {
    ...
    f   <- y ~ 1
    mod <- try(suppressMessages(gamlss2(f, family=BS9)), silent = TRUE)
...
archivos <- list.files(pattern = "^BS9_Sim_WCov.*\\.txt$", 
...
datos$case <- with(datos, 
                   ifelse(mu==1 & sigma==1.5, 1, 
                          ifelse(mu==1 & sigma==1.9, 2,
                                 ifelse(mu==2 & sigma==1.5, 3, 4))))
...
ggsave(filename="C:/Users/davil/Desktop/BS-Parametrizations/Simulaciones/Figs/bias_BS9_Sim_WCov.pdf", width=12, height=6,
...
ggsave(filename="C:/Users/davil/Desktop/BS-Parametrizations/Simulaciones/Figs/mse_BS9_Sim_WCov.pdf", width=12, height=6,
