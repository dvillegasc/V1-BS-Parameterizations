...
gendat <- function(n) {
  ...
  sigma <- exp(1.2 + 0.8 * x2) #  5  approximately
...
parSim(
  ...
  name = "C:/Users/davil/Desktop/BS-Parametrizations/Simulaciones/Simuls/BS12_Sim_Cov",# Name of the file
  nCores = 6,                       # Number of cores to use
  progressbar = TRUE,               # Progress bar
  export = c("gendat", "dBS", "pBS", "qBS", "rBS", "hBS", "dBS12", "pBS12", "qBS12", "rBS12", "hBS12", "BS12"),
  ...
  expression = {
    ...
    f   <- y ~ x1 | x2 
    mod <- try(suppressMessages(gamlss2(f, family=BS12, data=dat)), silent = TRUE)
...
archivos <- list.files(pattern = "^BS12_Sim_Cov.*\\.txt$", 
...
ggsave(filename = "C:/Users/davil/Desktop/BS-Parametrizations/Simulaciones/Figs/bias_mse_BS12_Sim_Cov.pdf", 
