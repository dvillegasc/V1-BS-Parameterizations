...
gendat <- function(n) {
  ...
  sigma <- exp(-0.9 + 0.4 * x2) #  10  approximately
...
parSim(
  ...
  name = "C:/Users/davil/Desktop/BS-Parametrizations/Simulaciones/Simuls/BS6_Sim_Cov",# Name of the file
  nCores = 6,                       # Number of cores to use
  progressbar = TRUE,               # Progress bar
  export = c("gendat", "dBS", "pBS", "qBS", "rBS", "hBS", "dBS6", "pBS6", "qBS6", "rBS6", "hBS6", "BS6"),
  ...
  expression = {
    ...
    f   <- y ~ x1 | x2 
    mod <- try(suppressMessages(gamlss2(f, family=BS6, data=dat)), silent = TRUE)
...
archivos <- list.files(pattern = "^BS6_Sim_Cov.*\\.txt$", 
...
dat <- datos %>% group_by(n) %>% 
  summarise(nobs = n(),
            ...
            bias_g0 = mean(gamma_0_hat - (-0.9), trim=trim, na.rm=TRUE),
            ...
            mse_g0 = mean((gamma_0_hat - (-0.9))^2, trim=trim, na.rm=TRUE),
...
ggsave(filename = "C:/Users/davil/Desktop/BS-Parametrizations/Simulaciones/Figs/bias_mse_BS6_Sim_Cov.pdf", 
