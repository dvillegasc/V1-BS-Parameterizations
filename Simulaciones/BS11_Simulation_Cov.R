...
gendat <- function(n) {
  x1 <- runif(n)
  x2 <- runif(n)
  mu    <- exp(0.2 + 0.4 * x1) # 1 approximately
  sigma <- exp(0.3 + 0.4 * x2) #  2  approximately
...
parSim(
  ...
  name = "C:/Users/davil/Desktop/BS-Parametrizations/Simulaciones/Simuls/BS11_Sim_Cov",# Name of the file
  nCores = 6,                       # Number of cores to use
  progressbar = TRUE,               # Progress bar
  export = c("gendat", "dBS", "pBS", "qBS", "rBS", "hBS", "dBS11", "pBS11", "qBS11", "rBS11", "hBS11", "BS11"),
  ...
  expression = {
    ...
    f   <- y ~ x1 | x2 
    mod <- try(suppressMessages(gamlss2(f, family=BS11, data=dat)), silent = TRUE)
...
archivos <- list.files(pattern = "^BS11_Sim_Cov.*\\.txt$", 
...
ggsave(filename = "C:/Users/davil/Desktop/BS-Parametrizations/Simulaciones/Figs/bias_mse_BS11_Sim_Cov.pdf", 
