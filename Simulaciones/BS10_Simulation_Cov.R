...
gendat <- function(n) {
  x1 <- runif(n)
  x2 <- runif(n)
  mu    <- exp(0.1 + 0.2 * x1) # 1.2 approximately
  sigma <- exp(1.0 + 1.2 * x2) #  5  approximately
...
parSim(
  ...
  name = "C:/Users/davil/Desktop/BS-Parametrizations/Simulaciones/Simuls/BS10_Sim_Cov",# Name of the file
  nCores = 6,                       # Number of cores to use
  progressbar = TRUE,               # Progress bar
  export = c("gendat", "dBS", "pBS", "qBS", "rBS", "hBS", "dBS10", "pBS10", "qBS10", "rBS10", "hBS10", "BS10"),
  ...
  expression = {
    ...
    f   <- y ~ x1 | x2 
    mod <- try(suppressMessages(gamlss2(f, family=BS10, data=dat)), silent = TRUE)
...
archivos <- list.files(pattern = "^BS10_Sim_Cov.*\\.txt$", 
...
dat <- datos %>% group_by(n) %>% 
  summarise(nobs = n(),
            ...
            bias_b0 = mean(beta_0_hat - (0.1), trim=trim, na.rm=TRUE),
            bias_b1 = mean(beta_1_hat - (0.2), trim=trim, na.rm=TRUE),
            bias_g0 = mean(gamma_0_hat - (1.0), trim=trim, na.rm=TRUE),
            bias_g1 = mean(gamma_1_hat - (1.2), trim=trim, na.rm=TRUE),
            
            mse_b0 = mean((beta_0_hat - (0.1))^2, trim=trim, na.rm=TRUE),
            mse_b1 = mean((beta_1_hat - (0.2))^2, trim=trim, na.rm=TRUE),
            mse_g0 = mean((gamma_0_hat - (1.0))^2, trim=trim, na.rm=TRUE),
            mse_g1 = mean((gamma_1_hat - (1.2))^2, trim=trim, na.rm=TRUE)
  )
...
ggsave(filename = "C:/Users/davil/Desktop/BS-Parametrizations/Simulaciones/Figs/bias_mse_BS10_Sim_Cov.pdf", 
